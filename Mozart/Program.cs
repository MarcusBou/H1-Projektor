...
namespace Mozart
{
    class Program
    {
        static int[,] Minuetten = new int[16, 11] {
        ...
        static int[,] Trioen = new int[16, 6] {
        ...
        static string filepath = @"C:\Users\marc8847\Documents\Wave\";
        ...
        static void Main(string[] args)
        {
            for (int i = 0; i < Minuetten.GetUpperBound(0) + 1; i++)
            {
                ...
                soundPlayer.SoundLocation = $@"{filepath}M{Minuetten[i, Dice1 + Dice2 - 2]}{fileType}";
                soundPlayer.Load();
            ...
            for (int i = 0; i < Trioen.GetUpperBound(0) + 1; i++)
            {
                ...
                soundPlayer.SoundLocation = $@"{filepath}T{Trioen[i, Dice1 - 1]}{fileType}";
                soundPlayer.Load();
                Console.WriteLine("Der spilles: T" + Trioen[i, Dice1 - 1]);
                soundPlayer.PlaySync();
            }
